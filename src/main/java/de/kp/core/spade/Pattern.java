...
import de.kp.core.spade.creator.PatternCreator;
...
public class Pattern implements Comparable<Pattern> {
    ...
    public String serialize() {
        ...
        for (int i = 0; i < elements.size(); i++) {
        	...
        	ItemAbstractionPair pair = elements.get(i);
            ...
            if (i==elements.size()-1){
            	 ...
            	 */
                if(i!=0)
                    result.append(pair.toStringToFile());
                else
                	/**
                	 * The first item is the last item; so this pattern
                	 * covers only a single item
                	 */
                    result.append(pair.getItem());
                /**
                 * Inidicate that the itemset has finished
                 */
                result.append(" -1");
            	...
            	/**
            	 * Process the first pair; here we know that there
            	 * is more than one item (see above)
            	 */
                result.append(pair.getItem());
            } else {
                result.append(pair.toStringToFile());
