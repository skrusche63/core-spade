...
public class AbstractionCreatorQualitative extends AbstractionCreator {

    private static AbstractionCreatorQualitative instance = null;

    public static void sclear() {
        instance=null;
    }

    private AbstractionCreatorQualitative() {
    }

    public static AbstractionCreatorQualitative getInstance() {
        if (instance == null) {
            instance = new AbstractionCreatorQualitative();
        }
        return instance;
    }
     ...
     */
    @Override
    public AbstractionGeneric createDefaultAbstraction() {
     ...
     */
    public AbstractionGeneric createAbstraction(boolean hasEqualRelation) {
    ...
    public List<EquivalenceClass> getFrequentSize2Sequences(List<Sequence> sequences, IdListCreator idListCreator) {
        ...
        for (Sequence seq : sequences) {
        	/**
        	 * A sequence consists of itemsets (see input format)
        	 */
            ...
            for (int i = 0; i < itemsets.size(); i++) {
                ...
                for (int j = 0; j < currentItemset.size(); j++) {
                	/**
                	 * Process an itemset of a selected sequence
                	 */
                    ...
                    for (int k = j + 1; k < currentItemset.size(); k++) {
                        ...
                        ItemAbstractionPair pair2 = new ItemAbstractionPair(item2, AbstractionQualitative.create(true));
                    ...
                    for (int k = i + 1; k < itemsets.size(); k++) {
                        ...
                        for (int n = 0; n < nextItemset.size(); n++) {
                            ...
                            ItemAbstractionPair pair2 = new ItemAbstractionPair(item2, AbstractionQualitative.create(false));
    ...
    public Pattern getSubpattern(Pattern extension, int index) {
        ...
        AbstractionGeneric abstraction = null;
        ...
        for (int i = 0; i < extension.size(); i++) {
            if (i != index) {
                ...
                if (index == 0) {
                    ...
                    AbstractionQualitative abstractionOfRemovedPair = (AbstractionQualitative) extension.getIthElement(i).getAbstraction();                    
                        ...
                        for (int k = 0; k < appearances2.size(); k++) {
                            ...
                            if (item2Apperance == item1Appearance) {
                                if (-item2.compareTo(item1) == 1) {
                                    pair2 = new ItemAbstractionPair(item2, AbstractionQualitative.create(true));
                                }
                            } else if (item2Apperance > item1Appearance) {
                                pair2 = new ItemAbstractionPair(item2, AbstractionQualitative.create(false));
                            }
                            if (pair2 != null) {
